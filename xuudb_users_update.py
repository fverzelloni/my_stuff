...
users_to_skip = ['bp000094', 'bp000095', 'bp000096', 'bp000235', 'ich019sa', 'bp000095', 'bp000094:fverzell', 'fverzell:bp000094', 'lbologna:bp000029', 'bp000029', 'bp000167'  ]
http_srv = 'someo.cscs.ch'

# Functions
def get_ump_users():
    '''Get active users list from UMP RestAPI'''
    payload = {'projectGroupStatus': 'ACCESSIBLE+OPEN+EXPIRING', 'userStatus': 'ACTIVE+EXPIRING', 'facilityId': facility }
    #payload = {'status': 'ACCESSIBLE+OPEN+EXPIRING', 'facilityId': facility }
    #payload = {}
    #start_function = time.time()
    resp = requests.get('{}v3/bulk/active-users-by-facility'.format(ump_url), params=payload, auth=(ump_username, ump_password), timeout=10)
    #print(resp)
    #resp = requests.get('{}v3/bulk/active-users-by-facility'.format(ump_url), params=payload, auth=(ump_username, ump_password), timeout=10)
...
#def get_hbp_data():
    ...
    #resp = requests.get('{}hpcmanagement/v1/accounts'.format(hbp_url), auth=(hbp_username, hbp_password), timeout=20, verify=False)
    #end_function = time.time()
    if resp.status_code != 200:
...
def get_xuudb_users():
    ...
    start_function = time.time()
    subprocess.call('/opt/Unicore/default/xuudb/bin/admin.sh export /tmp/xuudb_db.csv', shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)

# Generate parsed xuudb_db.csv list
get_xuudb_users()
bp_users_list_xuudb = []
with open('/tmp/xuudb_db.csv', 'r') as f:
    ...
    if len(users_to_add) != 0:
        for user in users_to_add:
            if user not in users_to_skip:
                ...
                users_list_add.append(f'/opt/Unicore/default/xuudb/bin/admin.sh adddn  {unicore_dn} "CN={displayname} {uid},O=hbp" {user} user')
                users_list_add.append(f'/opt/Unicore/default/xuudb/bin/admin.sh adddn  {unicore_dn} "UID={email}" {user} user')
...
#df = pd.read_csv('/tmp/xuudb_db.csv', sep=';' )

# Save to file
#df.to_html('/tmp/xuudb_prod.html')

# Assign to string
#htmTable = df.to_html()

# Copy HTML to remote server
#p = subprocess.Popen(['scp', '/tmp/xuudb_prod.html', 'root@'+ http_srv +':/var/www/html/confluence/xuudb_prod.html'])
#sts = p.wait()
########################################

# Delete .csv file from /tmp
os.remove('/tmp/xuudb_db.csv')
#os.remove('/tmp/xuudb_prod.html')
